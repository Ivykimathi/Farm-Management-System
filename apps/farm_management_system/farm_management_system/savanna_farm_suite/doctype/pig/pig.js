...
function fetchUOMForFeeds(tooltipData) {
    return new Promise((resolve, reject) => {
        ...
        frappe.call({
            method: 'farm_management_system.savanna_farm_suite.doctype.pig.pig.get_animal_feed_uoms',
            args: {
...
function render_collection_chart(frm) {
    ...
    frappe.call({
        method: "farm_management_system.savanna_farm_suite.doctype.pig.pig.get_collection_data",
        args: {
            pig: frm.doc.name
        },
        callback: function(r) {
...
function renderTreatmentChart(frm) {
	...
	if (!frm.doc || !frm.doc.name) {
		$wrap.html('<div class="text-muted">No Pig Selected.</div>');
		return;
	}
	...
	frappe.call({
		method: 'farm_management_system.savanna_farm_suite.doctype.pig.pig.get_treatment_chart_data',
		args: { pig: frm.doc.name },
		freeze: true,
		...
		if (!dates.length || !vaccines.length) {
			$wrap.html('<div class="text-muted">No treatment / vaccination logs found for this pig.</div>');
			return;
		}

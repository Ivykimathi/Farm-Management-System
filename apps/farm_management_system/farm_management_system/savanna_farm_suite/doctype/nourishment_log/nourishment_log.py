...
class NourishmentLog(Document):
    ...
    def on_submit(self):
        ...
        if self.log_intended_for_cattle_shed:
            self._update_shed_and_animals("Cattle Shed", self.log_intended_for_cattle_shed, "Cattle", "cow_shed")
        if getattr(self, "log_intended_for_sheep_shed", None):
            self._update_shed_and_animals("Sheep Shed", self.log_intended_for_sheep_shed, "Sheep", "sheep_shed")
        if getattr(self, "log_intended_for_rabbit_shed", None):
            self._update_shed_and_animals("Rabbit Shed", self.log_intended_for_rabbit_shed, "Rabbit", "rabbit_shed")
        if getattr(self, "log_intended_for_pig_shed", None):
            self._update_shed_and_animals("Pig Shed", self.log_intended_for_pig_shed, "Pig", "pig_shed")
        if getattr(self, "log_intended_for_goat_shed", None):
            self._update_shed_and_animals("Goat Shed", self.log_intended_for_goat_shed, "Goat", "goat_shed")

    def _update_shed_and_animals(self, shed_doctype, shed_name, animal_doctype, animal_shed_field):
        try:
            # Update Shed document
            shed_doc = frappe.get_doc(shed_doctype, shed_name)
            shed_doc.append("feeding_logs", {
            ...
            # Update individual animal documents in this shed
            animal_list = frappe.get_all(animal_doctype, 
                                        filters={animal_shed_field: shed_name},
                                        fields=["name"])
            
            updated_count = 0
            for animal in animal_list:
                animal_doc = frappe.get_doc(animal_doctype, animal.name)
                animal_doc.append("feeding_log", {
                    "date_fed": self.date_of_nourishment,
                    "fed_on": self.feed_issued,
            ...
            frappe.msgprint(
                _("Successfully updated feeding logs for {0} animals in {1} {2}").format(
                    updated_count, shed_name, shed_doctype
                ),
            ...
            frappe.log_error(
                _("Error updating feeding logs for {0} {1}: {2}").format(
                    shed_doctype, shed_name, str(e)
                ),
                "Nourishment Log on_submit"
            )
            frappe.msgprint(
                _("Failed to update feeding logs for {0}. Please check error logs.").format(shed_doctype),

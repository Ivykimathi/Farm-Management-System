...
class PoultryBatches(Document):
	...
	def validate(self):
		# Backfill `poultry_batch` on each shed-block row with this batch's name so
		# the denormalized link stays consistent with the parent.
...
def _resolve_batch_for_shed(shed):
    ...
    Checks the new many-to-many `Poultry Batch Blocks` child table first, then falls
    back to the legacy single Link field on Poultry Shed.
    """
    if not shed:
        return None
    ...
    legacy_batch = frappe.db.get_value("Poultry Shed", shed, "poultry_batch")
    if legacy_batch:
        status = frappe.db.get_value("Poultry Batches", legacy_batch, "batch_status")
        if status == "Active":
            return legacy_batch
    return None
...
@frappe.whitelist()
def get_batches_for_shed(shed, txt=None, searchfield=None, start=0, page_len=20, filters=None):
    """
    Get active poultry batches associated with a specific shed.
    Used for filtering in Make Collection dialog.
    """
    batch = _resolve_batch_for_shed(shed)
    if batch:
        return [[batch]]
    return []
...
def create_collection_entry(date_of_collection, rows, selection_mode=None, poultry_shed=None, collected_by=None):
    ...
    If selection_mode is "Shed", poultry_shed may be provided and the batch is inferred from the shed.
    ...
    if selection_mode == 'Shed' and poultry_shed:
        batch = _resolve_batch_for_shed(poultry_shed)
        if not batch:
            frappe.throw(_("Selected Poultry Shed is not linked to an active Poultry Batch."), frappe.ValidationError)
        for r in rows:
            if isinstance(r, dict) and not r.get("poultry_batch"):
                r["poultry_batch"] = batch

    # Allow individual rows to specify their poultry_batch or poultry_shed mapping.
    for r in rows:
        if isinstance(r, dict) and r.get("poultry_shed") and not r.get("poultry_batch"):
            batch = _resolve_batch_for_shed(r.get("poultry_shed"))
            if not batch:
    ...
    for batch, sub_rows in groups.items():
        ...
        # Remove any incomplete rows from poultry_shed_blocks to avoid validation errors
        if hasattr(pb, 'poultry_shed_blocks') and pb.poultry_shed_blocks:
            pb.poultry_shed_blocks = [row for row in pb.poultry_shed_blocks 
                                     if row.get('poultry_batch') and row.get('poultry_shed')]
        pb.flags.ignore_validate = True

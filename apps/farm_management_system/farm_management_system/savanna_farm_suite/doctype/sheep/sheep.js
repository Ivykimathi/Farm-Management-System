...
function fetchUOMForFeeds(tooltipData) {
    return new Promise((resolve, reject) => {
        ...
        frappe.call({
            method: 'farm_management_system.savanna_farm_suite.doctype.sheep.sheep.get_animal_feed_uoms',
            args: {
...
function render_collection_chart(frm) {
    ...
    frappe.call({
        method: "farm_management_system.savanna_farm_suite.doctype.sheep.sheep.get_collection_data",
        args: {
            sheep: frm.doc.name
        },
        callback: function(r) {
...
function renderTreatmentChart(frm) {
	...
	if (!frm.doc || !frm.doc.name) {
		$wrap.html('<div class="text-muted">No Sheep Selected.</div>');
		return;
	}
	...
	frappe.call({
		method: 'farm_management_system.savanna_farm_suite.doctype.sheep.sheep.get_treatment_chart_data',
		args: { sheep: frm.doc.name },
		freeze: true,
		...
		if (!dates.length || !vaccines.length) {
			$wrap.html('<div class="text-muted">No treatment / vaccination logs found for this sheep.</div>');
			return;
		}

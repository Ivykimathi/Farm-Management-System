...
def execute(filters=None):
    ...
    milker_filter = filters.get("milker")

    # aggregation: key = (cow_name, iso_date, time_of_collection, milker, product_collected, uom) -> total_qty
    agg = {}
    ...
    for c in cattle_list:
        ...
        for row in rows:
            ...
            if isinstance(row, dict):
                ...
                row_time = row.get("time_of_collection")
                milker = row.get("milker") or ""
                ...
                row_time = getattr(row, "time_of_collection", None)
                milker = getattr(row, "milker", "") or ""
            ...
            # milker filter
            if milker_filter and milker != milker_filter:
                continue
    ...
    unique_milkers = set()
    for key in agg.keys():
        cow_name, iso_date, row_time, milker, product_collected, uom = key
        if milker:
            unique_milkers.add(milker)

    milker_names = {}
    if unique_milkers:
        employees = frappe.get_all("Employee", filters={"name": ["in", list(unique_milkers)]}, fields=["name", "employee_name"])
        milker_names = {d.name: d.employee_name for d in employees}
    ...
    for key in sorted_keys:
        ...
        data.append([
            ...
            row_time or "",  # Time of Collection (Time string)
            milker_names.get(milker, milker),  # Milker (Data: full name)

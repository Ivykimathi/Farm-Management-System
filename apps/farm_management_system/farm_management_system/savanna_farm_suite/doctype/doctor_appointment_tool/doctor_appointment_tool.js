...
frappe.ui.form.on("Doctor Appointment Tool", {
    setup_buttons(frm) {
        ...
        frm.add_custom_button(__('Add Appointment'), function () {
            openAddAppointmentDialog(frm);
        });

        frm.add_custom_button(__('Calendar View'), function () {
            if (frm.doc.specify_doctor) {
                window.location.href = '/app/farm-activity-calend?doctor=' + encodeURIComponent(frm.doc.specify_doctor);
            } else {
                frappe.msgprint(__('Please specify a Doctor first to view their calendar.'));
            }
        });
    },
        ...
        if (frm.doc.specify_doctor) {
            ...
            frappe.call({
                ...
                args: {
                    ...
                    fields: ['name', 'specify_type_of_treatment', 'treatment_date', 'animal_under_medication', 'status', 'doctor', 'poultry_batch_under_treatment', 'cattle_shed_under_treatment', 'specific_cattle_under_treatment', 'sheep_shed_under_treatment', 'specific_sheep_under_treatment', 'rabbit_shed_under_treatment', 'specific_rabbit_under_treatment', 'pig_shed_under_treatment', 'specific_pig_under_treatment', 'goat_shed_under_treatment', 'specific_goat_under_treatment', 'vaccine_used', 'qty_vaccine', 'doctors_purchase_order_based_on_appointment_fee'],
...
function renderTreatmentLogs(frm, data) {
    frm.__selected_extra_columns = frm.__selected_extra_columns || [];

    const available_columns = [
        { fieldname: 'doctor', label: __('Doctor') },
        { fieldname: 'poultry_batch_under_treatment', label: __('Poultry Batch') },
        { fieldname: 'cattle_shed_under_treatment', label: __('Cattle Shed') },
        { fieldname: 'specific_cattle_under_treatment', label: __('Specific Cattle') },
        { fieldname: 'sheep_shed_under_treatment', label: __('Sheep Shed') },
        { fieldname: 'specific_sheep_under_treatment', label: __('Specific Sheep') },
        { fieldname: 'rabbit_shed_under_treatment', label: __('Rabbit Shed') },
        { fieldname: 'specific_rabbit_under_treatment', label: __('Specific Rabbit') },
        { fieldname: 'pig_shed_under_treatment', label: __('Pig Shed') },
        { fieldname: 'specific_pig_under_treatment', label: __('Specific Pig') },
        { fieldname: 'goat_shed_under_treatment', label: __('Goat Shed') },
        { fieldname: 'specific_goat_under_treatment', label: __('Specific Goat') },
        { fieldname: 'vaccine_used', label: __('Vaccine Used') },
        { fieldname: 'qty_vaccine', label: __('Qty Vaccine') },
        { fieldname: 'doctors_purchase_order_based_on_appointment_fee', label: __('Purchase Order') }
    ];

    let col_html = `
        <div class="column-selector mb-3 p-3 bg-light rounded" style="border: 1px solid #ddd;">
            <div class="font-weight-bold mb-2">${__('Select Extra Columns to Display (Max 9)')}</div>
            <div class="d-flex flex-wrap" style="gap: 15px;">
    `;
    available_columns.forEach(col => {
        const checked = frm.__selected_extra_columns.includes(col.fieldname) ? 'checked' : '';
        col_html += `
            <label class="d-flex align-items-center mb-0" style="cursor: pointer; user-select: none;">
                <input type="checkbox" class="extra-column-check mr-1" value="${col.fieldname}" ${checked}> 
    ...
    frm.__selected_extra_columns.forEach(fieldname => {
        let col_def = available_columns.find(c => c.fieldname === fieldname);
        if (col_def) {
            html += `<th>${col_def.label}</th>`;
        }
    });

    html += `<th>${__('Actions')}</th></tr></thead><tbody>`;

    if (data.length === 0) {
        html += `
            <tr>
                <td colspan="${5 + frm.__selected_extra_columns.length}" class="text-center text-muted">
        ...
        data.forEach(row => {
            html += `<tr>`;
            html += `<td>${frappe.datetime.str_to_user(row.treatment_date) || ''}</td>`;
            html += `<td>${row.specify_type_of_treatment || ''}</td>`;
            html += `<td>${row.animal_under_medication || ''}</td>`;
            html += `<td><span class="badge badge-secondary">${row.status || ''}</span></td>`;

            frm.__selected_extra_columns.forEach(fieldname => {
                html += `<td>${row[fieldname] || ''}</td>`;
            });
    ...
    // Bind checkbox logic
    frm.fields_dict.html_riid.$wrapper.find('.extra-column-check').change(function () {
        const fieldname = $(this).val();
        if ($(this).is(':checked')) {
            if (frm.__selected_extra_columns.length >= 9) {
                $(this).prop('checked', false);
                frappe.msgprint(__('You can select a maximum of 5 extra columns.'));
                return;
            }
            frm.__selected_extra_columns.push(fieldname);
        } else {
            frm.__selected_extra_columns = frm.__selected_extra_columns.filter(f => f !== fieldname);
        }
        renderTreatmentLogs(frm, data);
    });
...
function openAddAppointmentDialog(frm) {
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            {
                fieldname: 'sheep_shed',
                fieldtype: 'Link',
                depends_on: "eval:doc.animal == 'Sheep'",
                mandatory_depends_on: "eval:doc.animal == 'Sheep'",
                label: __('Specify Sheep Shed'),
                options: 'Sheep Shed'
            },
            {
                fieldname: 'sheep',
                fieldtype: 'Link',
                depends_on: "eval:doc.sheep_shed",
                label: __('Specify Sheep under Treatment (OPTIONAL)'),
                options: 'Sheep',
            },
            {
                fieldname: 'rabbit_shed',
                fieldtype: 'Link',
                depends_on: "eval:doc.animal == 'Rabbit'",
                mandatory_depends_on: "eval:doc.animal == 'Rabbit'",
                label: __('Specify Rabbit Shed'),
                options: 'Rabbit Shed'
            },
            {
                fieldname: 'rabbit',
                fieldtype: 'Link',
                depends_on: "eval:doc.rabbit_shed",
                label: __('Specify Rabbit under Treatment (OPTIONAL)'),
                options: 'Rabbit',
            },
            {
                fieldname: 'pig_shed',
                fieldtype: 'Link',
                depends_on: "eval:doc.animal == 'Pig'",
                mandatory_depends_on: "eval:doc.animal == 'Pig'",
                label: __('Specify Pig Shed'),
                options: 'Pig Shed'
            },
            {
                fieldname: 'pig',
                fieldtype: 'Link',
                depends_on: "eval:doc.pig_shed",
                label: __('Specify Pig under Treatment (OPTIONAL)'),
                options: 'Pig',
            },
            {
                fieldname: 'goat_shed',
                fieldtype: 'Link',
                depends_on: "eval:doc.animal == 'Goat'",
                mandatory_depends_on: "eval:doc.animal == 'Goat'",
                label: __('Specify Goat Shed'),
                options: 'Goat Shed'
            },
            {
                fieldname: 'goat',
                fieldtype: 'Link',
                depends_on: "eval:doc.goat_shed",
                label: __('Specify Goat under Treatment (OPTIONAL)'),
                options: 'Goat',
            },
            {
...
function createAppointment(frm, values, dialog) {
    ...
    frappe.call({
        ...
        args: {
            ...
            sheep_shed: values.sheep_shed,
            sheep: values.sheep,
            rabbit_shed: values.rabbit_shed,
            rabbit: values.rabbit,
            pig_shed: values.pig_shed,
            pig: values.pig,
            goat_shed: values.goat_shed,
            goat: values.goat
        },
        callback: function (r) {
            frappe.dom.unfreeze();

...
def create_collection_entry(cattle=None, date_of_collection=None, rows=None):
    ...
    Accepts:
      ...
      - rows: list of dicts OR JSON string. Each row may contain:
          { "cattle": "COW-0001", "animal_product": "Milk", "default_uom": "Ltr", "quantity_collected": 2.5, "time_of_collection": "08:30:00" }
        OR shed-based rows:
          { "cattle_shed": "SHED-001", "animal_product": "Milk", "default_uom": "Ltr", "quantity_collected": 2.5, "time_of_collection": "08:30:00" }
    ...
    def _append_rows_to_cattle(cattle_name, rows_for_cattle):
        ...
        for r in rows_for_cattle:
            ...
            pb.append("production_log", {
                    ...
                    "time_of_collection": r.get("time_of_collection") or nowtime(),
                    "milker": r.get("milker") or r.get("employee") or "",
    ...
    # Determine whether rows contain shed-based or cattle-based keys
    rows_have_shed_key = any((isinstance(r, dict) and r.get("cattle_shed")) for r in rows)
    rows_have_cattle_key = any((isinstance(r, dict) and r.get("cattle")) for r in rows)

    if rows_have_shed_key:
        # ── Shed-specific mode ──
        # Group rows by cattle_shed
        grouped_by_shed = {}
        for r in rows:
            if not isinstance(r, dict):
                continue
            shed_name = r.get("cattle_shed")
            if not shed_name:
                continue
            grouped_by_shed.setdefault(shed_name, []).append(r)

        for shed_name, shed_rows in grouped_by_shed.items():
            # Find all Cattle in this shed
            cattle_in_shed = frappe.get_all(
                "Cattle",
                filters={"cow_shed": shed_name, "has_been_culled": 0},
                fields=["name"],
                limit_page_length=2000
            )
 
            if not cattle_in_shed:
                frappe.log_error(f"No active cattle found in shed {shed_name}", "create_collection_entry")
                continue

            animal_count = len(cattle_in_shed)
            ...
            # For each cattle in the shed, apply proportionate product rows
            for cattle_rec in cattle_in_shed:
                cattle_name = cattle_rec.name
                try:
                    _append_rows_to_cattle(cattle_name, adjusted_rows)
                except Exception as e:
                    frappe.log_error(f"Error appending rows to Cattle {cattle_name} (shed {shed_name}): {e}", "create_collection_entry")
                    continue
    ...
    elif cattle and not rows_have_cattle_key:
        # ── Single cattle mode (top-level) ──
        ...
        # ── Per-cattle rows mode ──
        grouped = {}
        for r in rows:
            if not isinstance(r, dict):
                continue
            cattle_name = r.get("cattle") or r.get("cow")
            if not cattle_name:
...
def cull_animal(animals=None, date_of_cull=None):
    """
    Cull one or more Cattle animals.
    animals: JSON list of Cattle names to cull
    ...
    for animal_name in animals:
        try:
            ...
            doc = frappe.get_doc("Cattle", animal_name)
            ...
            if asset_list:
                ...
                if asset_status not in ("Scrapped", "Sold", "Cancelled", "Capitalized"):
                    try:
                        ...
                        frappe.log_error(
                            f"Failed to scrap Asset {asset_doc_name} for Cattle {animal_name}: {e}",
                            "cull_animal"
                        )
            ...
            frappe.log_error(
                f"Error culling Cattle {animal_name}: {e}",
                "cull_animal"
            )
            results.append({
                "animal": animal_name,

...
class TreatmentandVaccinationLogs(Document):
	...
	def update_animal_logs(self):
		"""
		Update treatment logs in shed and/or individual animal documents
		with vaccine information from the current document.
		Supports: Cattle, Sheep, Rabbit, Pig, Goat
		"""
		# Config: (shed_field, shed_doctype, specific_field, specific_doctype, label)
		animal_configs = [
			('cattle_shed_under_treatment', 'Cattle Shed', 'specific_cattle_under_treatment', 'Cattle', 'cattle shed'),
			('sheep_shed_under_treatment', 'Sheep Shed', 'specific_sheep_under_treatment', 'Sheep', 'sheep shed'),
			('rabbit_shed_under_treatment', 'Rabbit Shed', 'specific_rabbit_under_treatment', 'Rabbit', 'rabbit shed'),
			('pig_shed_under_treatment', 'Pig Shed', 'specific_pig_under_treatment', 'Pig', 'pig shed'),
			('goat_shed_under_treatment', 'Goat Shed', 'specific_goat_under_treatment', 'Goat', 'goat shed'),
		]

		for shed_field, shed_doctype, specific_field, specific_doctype, label in animal_configs:
			try:
				# Update shed treatment logs
				shed_name = getattr(self, shed_field, None)
				if shed_name:
					shed_doc = frappe.get_doc(shed_doctype, shed_name)
					shed_doc.append("treatment_logs", {
					...
					shed_doc.save(ignore_permissions=True)
					frappe.msgprint(
						_("Treatment log updated for {0} {1}").format(label, shed_name),
						indicator="green"
					)

				# Update individual animal treatment logs
				specific_name = getattr(self, specific_field, None)
				if specific_name:
					animal_doc = frappe.get_doc(specific_doctype, specific_name)
					animal_doc.append("treatment_table", {
						"treatment_date": self.treatment_date,
						"animal_vaccine_issued": self.vaccine_used,
						"treatment_conducted_by": self.doctor,
						"quantity_of_vaccine_issued": self.qty_vaccine,
					...
					frappe.msgprint(
						_("Treatment log updated for {0} {1}").format(specific_doctype.lower(), specific_name),
						indicator="green"
					)

			except Exception as e:
				frappe.log_error(
					_("Error updating {0} treatment logs for {1}: {2}").format(label, self.name, str(e)),
					"Animal Treatment Update"
				)
				frappe.msgprint(
					_("Failed to update {0} treatment logs. Please check error logs.").format(label),

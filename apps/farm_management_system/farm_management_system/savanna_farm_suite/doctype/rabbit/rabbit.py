...
class Rabbit(Document):
    def after_insert(self):
        ...
        if self.rabbit_shed:
            shed = frappe.get_doc("Rabbit Shed", self.rabbit_shed)
            current_count_str = shed.current_animal_count or "0 Rabbits"
            try:
            ...
            new_count = number + 1
            shed.current_animal_count = f"{new_count} Rabbits"
            shed.save(ignore_permissions=True)

    def create_asset(self):
        """Create Item and Asset records for the rabbit"""
        try:
            # Create Item first
            item = frappe.get_doc({
                "doctype": "Item",
            ...
            frappe.log_error(_("Error creating asset for rabbit {0}: {1}").format(self.name, str(e)))
            frappe.throw(_("Failed to create asset record. Please check error logs."))
    
    def generate_avatar(self):
        """Generate a unique avatar for the rabbit and attach it."""
        try:
            # Generate SVG code using multiavatar
            svg_code = multiavatar("A rabbit", None, None)

            # Create a File document for the SVG
            file_doc = frappe.get_doc({
                "doctype": "File",
                "file_name": f"avatar-{self.name}.svg",
                "attached_to_doctype": "Rabbit",
...
# file: farm_management_system/api.py  (or rabbit.py if you prefer)
import json
import frappe
from frappe.utils import getdate
...
@frappe.whitelist()
def create_collection_entry(rabbit=None, date_of_collection=None, rows=None):
    """
    Accepts:
      - rabbit (optional): single rabbit name (string)
      - date_of_collection: date string
      - rows: list of dicts OR JSON string. Each row may contain:
          { "rabbit": "Rabbit-0001", "animal_product": "Milk", "default_uom": "Ltr", "quantity_collected": 2.5 }
        OR shed-based rows:
          { "rabbit_shed": "SHED-001", "animal_product": "Milk", "default_uom": "Ltr", "quantity_collected": 2.5 }
        OR (if top-level rabbit used) rows may omit 'rabbit' and you should treat them all as for the top-level rabbit.
    """
    ...
    def _append_rows_to_animal(animal_name, rows_for_animal):
        if not animal_name:
            frappe.throw(_("Rabbit name required for these rows"), frappe.ValidationError)

        pb = frappe.get_doc("Rabbit", animal_name)
        if not pb:
            frappe.throw(_("Rabbit {0} not found").format(animal_name))
    ...
    rows_have_shed_key = any((isinstance(r, dict) and r.get("rabbit_shed")) for r in rows)
    rows_have_animal_key = any((isinstance(r, dict) and r.get("rabbit")) for r in rows)
    ...
    if rows_have_shed_key:
        ...
        for shed_name, shed_rows in grouped_by_shed.items():
            animals_in_shed = frappe.get_all(
                "Rabbit",
                filters={"rabbit_shed": shed_name, "has_been_culled": 0},
                fields=["name"],
                limit_page_length=2000
            )

            if not animals_in_shed:
                frappe.log_error(f"No active rabbit found in shed {shed_name}", "create_collection_entry")
                continue
            ...
            for animal_rec in animals_in_shed:
                ...
                try:
                    ...
                    frappe.log_error(f"Error appending rows to Rabbit {animal_name} (shed {shed_name}): {e}", "create_collection_entry")
                    continue
                ...
                for adj in adjusted_rows:
                    try:
                        ...
                        sle = create_stock_ledger_entry_collections(product_collected, qty, reference_doctype="Rabbit", reference_name=animal_name)
    ...
    elif rabbit and not rows_have_animal_key:
        pb = _append_rows_to_animal(rabbit, rows)

        for r in rows:
            try:
                product_collected = r.get("animal_product") or r.get("animal_products") or r.get("product")
                qty = flt(r.get("quantity_collected") or 0.0)
                sle = create_stock_ledger_entry_collections(product_collected, qty, reference_doctype="Rabbit", reference_name=rabbit)
                sle_results.append(sle)
            except Exception as e:
                frappe.log_error(f"create_stock_ledger_entry_collections failed for {product_collected}: {e}", "create_collection_entry")
    else:
        ...
        for r in rows:
            if not isinstance(r, dict):
                continue
            animal_name = r.get("rabbit")
            if not animal_name:
                frappe.throw(_("Row missing 'rabbit' value."), frappe.ValidationError)
        ...
        for animal_name, group_rows in grouped.items():
            try:
                ...
                frappe.log_error(f"Error appending rows to Rabbit {animal_name}: {e}", "create_collection_entry")
                continue
...
def cull_animal(animals=None, date_of_cull=None):
    """
    Cull one or more Rabbit animals.
    animals: JSON list of Rabbit names to cull
    ...
    for animal_name in animals:
        try:
            ...
            doc = frappe.get_doc("Rabbit", animal_name)
            ...
            if asset_list:
                ...
                if asset_status not in ("Scrapped", "Sold", "Cancelled", "Capitalized"):
                    try:
                        ...
                        frappe.log_error(
                            f"Failed to scrap Asset {asset_doc_name} for Rabbit {animal_name}: {e}",
                            "cull_animal"
                        )
            ...
            frappe.log_error(
                f"Error culling Rabbit {animal_name}: {e}",
                "cull_animal"
            )
            results.append({
                "animal": animal_name,
...
def create_stock_ledger_entry_collections(animal_product, qty_issued, reference_doctype=None, reference_name=None):
    ...
    sle_doc = frappe.get_doc({
        ...
        "voucher_type": reference_doctype or "Rabbit",
...
@frappe.whitelist()
def get_collection_data(rabbit):
    """
    ...
    try:
        ...
        doc = frappe.get_doc("Rabbit", rabbit)
        ...
        frappe.log_error(f"Attempted to get collection data for non-existent batch: {rabbit}")
...
@frappe.whitelist()
def get_treatment_chart_data(rabbit: str):
    """
    ...
    logs = frappe.get_all(
        ...
        filters={"specific_cattle_under_treatment": rabbit},
        fields=[
            "name",
            "treatment_date",
            "vaccine_used",

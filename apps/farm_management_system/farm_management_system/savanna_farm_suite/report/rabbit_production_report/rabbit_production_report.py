...
def execute(filters=None):
    ...
    columns = [
        "Rabbit:Link/Rabbit:180",
        "Rabbit Nickname:Data:140",
    ...
    rabbit_filter = filters.get("rabbit")
    product_filter = filters.get("product")

    # aggregation: key = (rabbit_name, iso_date, product_collected, uom) -> total_qty
    agg = {}

    # Preload list of rabbit (narrow by cow_filter if provided)
    if rabbit_filter:
        rabbit_list = frappe.get_all("Rabbit", filters={"name": rabbit_filter}, fields=["name"])
    else:
        rabbit_list = frappe.get_all("Rabbit", fields=["name"])

    # We'll cache nicknames per rabbit to avoid repeated fetches later
    nickname_map = {}

    for c in rabbit_list:
        try:
            rabbit_doc = frappe.get_doc("Rabbit", c.name)
        except Exception:
            continue

        # cache nickname
        nickname_map[c.name] = (rabbit_doc.get("add_nickname_optional") or "") if hasattr(rabbit_doc, "get") else (getattr(rabbit_doc, "add_nickname_optional", "") or "")

        rows = getattr(rabbit_doc, "production_log", []) or []
        for row in rows:
    ...
    for key in sorted_keys:
        rabbit_name, iso_date, product_collected, uom = key
        total_qty = agg[key]
        ...
        # Rabbit nickname from cache (may be empty string)
        nickname = nickname_map.get(rabbit_name, "")

        # Append row in the same order as columns
        data.append([
            rabbit_name,          # Rabbit (Link)
            nickname,          # Rabbit Nickname (Data)

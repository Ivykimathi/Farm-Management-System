...
def execute(filters=None):
    ...
    goat_filter = filters.get("goat")
    product_filter = filters.get("product")

    # aggregation: key = (goat_name, iso_date, product_collected, uom) -> total_qty
    agg = {}

    # Preload list of goat (narrow by cow_filter if provided)
    if goat_filter:
        goat_list = frappe.get_all("Goat", filters={"name": goat_filter}, fields=["name"])
    else:
        goat_list = frappe.get_all("Goat", fields=["name"])

    # We'll cache nicknames per goat to avoid repeated fetches later
    nickname_map = {}

    for c in goat_list:
        try:
            goat_doc = frappe.get_doc("Goat", c.name)
        except Exception:
            continue

        # cache nickname
        nickname_map[c.name] = (goat_doc.get("add_nickname_optional") or "") if hasattr(goat_doc, "get") else (getattr(goat_doc, "add_nickname_optional", "") or "")

        rows = getattr(goat_doc, "production_log", []) or []
        for row in rows:
    ...
    for key in sorted_keys:
        goat_name, iso_date, product_collected, uom = key
        total_qty = agg[key]
        ...
        # Goat nickname from cache (may be empty string)
        nickname = nickname_map.get(goat_name, "")

        # Append row in the same order as columns
        data.append([
            goat_name,          # Goat (Link)
            nickname,          # Goat Nickname (Data)

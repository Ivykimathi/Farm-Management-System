...
def execute(filters=None):
    ...
    pig_filter = filters.get("pig")
    product_filter = filters.get("product")

    # aggregation: key = (pig_name, iso_date, product_collected, uom) -> total_qty
    agg = {}

    # Preload list of pig (narrow by cow_filter if provided)
    if pig_filter:
        pig_list = frappe.get_all("Pig", filters={"name": pig_filter}, fields=["name"])
    else:
        pig_list = frappe.get_all("Pig", fields=["name"])

    # We'll cache nicknames per pig to avoid repeated fetches later
    nickname_map = {}

    for c in pig_list:
        try:
            pig_doc = frappe.get_doc("Pig", c.name)
        except Exception:
            continue

        # cache nickname
        nickname_map[c.name] = (pig_doc.get("add_nickname_optional") or "") if hasattr(pig_doc, "get") else (getattr(pig_doc, "add_nickname_optional", "") or "")

        rows = getattr(pig_doc, "production_log", []) or []
        for row in rows:
    ...
    for key in sorted_keys:
        pig_name, iso_date, product_collected, uom = key
        total_qty = agg[key]
        ...
        # Pig nickname from cache (may be empty string)
        nickname = nickname_map.get(pig_name, "")

        # Append row in the same order as columns
        data.append([
            pig_name,          # Pig (Link)
            nickname,          # Pig Nickname (Data)

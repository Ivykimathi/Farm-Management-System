...
class Sheep(Document):
    def after_insert(self):
        ...
        if self.sheep_shed:
            shed = frappe.get_doc("Sheep Shed", self.sheep_shed)
            current_count_str = shed.current_animal_count or "0 Sheep"
            try:
            ...
            new_count = number + 1
            shed.current_animal_count = f"{new_count} Sheep"
            shed.save(ignore_permissions=True)

    def create_asset(self):
        """Create Item and Asset records for the sheep"""
        try:
            # Create Item first
            item = frappe.get_doc({
                "doctype": "Item",
            ...
            frappe.log_error(_("Error creating asset for sheep {0}: {1}").format(self.name, str(e)))
            frappe.throw(_("Failed to create asset record. Please check error logs."))
    
    def generate_avatar(self):
        """Generate a unique avatar for the sheep and attach it."""
        try:
            # Generate SVG code using multiavatar
            svg_code = multiavatar("A sheep", None, None)

            # Create a File document for the SVG
            file_doc = frappe.get_doc({
                "doctype": "File",
                "file_name": f"avatar-{self.name}.svg",
                "attached_to_doctype": "Sheep",
...
# file: farm_management_system/api.py  (or sheep.py if you prefer)
import json
import frappe
from frappe.utils import getdate
...
@frappe.whitelist()
def create_collection_entry(sheep=None, date_of_collection=None, rows=None):
    """
    Accepts:
      - sheep (optional): single sheep name (string) — used when user selected one sheep at top-level
      - date_of_collection: date string
      - rows: list of dicts OR JSON string. Each row may contain:
          { "sheep": "Sheep-0001", "animal_product": "Milk", "default_uom": "Ltr", "quantity_collected": 2.5 }
        OR shed-based rows:
          { "sheep_shed": "SHED-001", "animal_product": "Milk", "default_uom": "Ltr", "quantity_collected": 2.5 }
        OR (if top-level sheep used) rows may omit 'sheep' and you should treat them all as for the top-level sheep.
    """
    ...
    def _append_rows_to_animal(animal_name, rows_for_animal):
        if not animal_name:
            frappe.throw(_("Sheep name required for these rows"), frappe.ValidationError)

        pb = frappe.get_doc("Sheep", animal_name)
        if not pb:
            frappe.throw(_("Sheep {0} not found").format(animal_name))

        for r in rows_for_animal:
    ...
    rows_have_shed_key = any((isinstance(r, dict) and r.get("sheep_shed")) for r in rows)
    rows_have_animal_key = any((isinstance(r, dict) and r.get("sheep")) for r in rows)

    if rows_have_shed_key:
        # ── Shed-specific mode ──
        grouped_by_shed = {}
        for r in rows:
            if not isinstance(r, dict):
                continue
            shed_name = r.get("sheep_shed")
            if not shed_name:
                continue
            grouped_by_shed.setdefault(shed_name, []).append(r)

        for shed_name, shed_rows in grouped_by_shed.items():
            animals_in_shed = frappe.get_all(
                "Sheep",
                filters={"sheep_shed": shed_name, "has_been_culled": 0},
                fields=["name"],
                limit_page_length=2000
            )

            if not animals_in_shed:
                frappe.log_error(f"No active sheep found in shed {shed_name}", "create_collection_entry")
                continue

            animal_count = len(animals_in_shed)

            # Build adjusted rows: split quantity proportionately across animals
            adjusted_rows = []
            for r in shed_rows:
                adj = dict(r)
                original_qty = flt(r.get("quantity_collected") or r.get("qty") or 0.0)
                adj["quantity_collected"] = flt(original_qty / animal_count) if animal_count else 0.0
                adjusted_rows.append(adj)

            for animal_rec in animals_in_shed:
                animal_name = animal_rec.name
                try:
                    _append_rows_to_animal(animal_name, adjusted_rows)
                except Exception as e:
                    frappe.log_error(f"Error appending rows to Sheep {animal_name} (shed {shed_name}): {e}", "create_collection_entry")
                    continue

                for adj in adjusted_rows:
                    try:
                        product_collected = adj.get("animal_product") or adj.get("animal_products") or adj.get("product")
                        qty = flt(adj.get("quantity_collected") or 0.0)
                        sle = create_stock_ledger_entry_collections(product_collected, qty, reference_doctype="Sheep", reference_name=animal_name)
    ...
    elif sheep and not rows_have_animal_key:
        pb = _append_rows_to_animal(sheep, rows)

        for r in rows:
            try:
                product_collected = r.get("animal_product") or r.get("animal_products") or r.get("product")
                qty = flt(r.get("quantity_collected") or 0.0)
                sle = create_stock_ledger_entry_collections(product_collected, qty, reference_doctype="Sheep", reference_name=sheep)
                sle_results.append(sle)
            except Exception as e:
                frappe.log_error(f"create_stock_ledger_entry_collections failed for {product_collected}: {e}", "create_collection_entry")
    else:
        ...
        for r in rows:
            if not isinstance(r, dict):
                continue
            animal_name = r.get("sheep")
            if not animal_name:
                frappe.throw(_("Row missing 'sheep' value. When collecting per-sheep, each row must include the sheep (Sheep) name."), frappe.ValidationError)
            grouped.setdefault(animal_name, []).append(r)

        for animal_name, group_rows in grouped.items():
            try:
                pb = _append_rows_to_animal(animal_name, group_rows)
            except Exception as e:
                frappe.log_error(f"Error appending rows to Sheep {animal_name}: {e}", "create_collection_entry")
                continue
...
@frappe.whitelist()
def cull_animal(animals=None, date_of_cull=None):
    """
    Cull one or more Sheep animals.
    animals: JSON list of Sheep names to cull
    date_of_cull: date string (defaults to today)
    For each animal:
      1. Set has_been_culled = 1, animal_culled_on = date
      2. Find linked Asset (by asset_name == animal name)
      3. Call scrap_asset to scrap the asset
    """
    if isinstance(animals, str):
        try:
            animals = json.loads(animals)
        except Exception:
            animals = frappe.parse_json(animals)

    animals = animals or []
    if not animals:
        frappe.throw(_("No animals selected for culling"), frappe.ValidationError)

    date_of_cull = date_of_cull or nowdate()
    results = []

    for animal_name in animals:
        try:
            # 1. Mark animal as culled
            doc = frappe.get_doc("Sheep", animal_name)
            doc.has_been_culled = 1
            doc.animal_culled_on = date_of_cull
            doc.save(ignore_permissions=True)

            # 2. Find linked Asset by asset_name
            asset_list = frappe.get_all(
                "Asset",
                filters={"asset_name": animal_name, "docstatus": 1},
                fields=["name", "status"],
                limit_page_length=1
            )
            ...
            asset_result = None
            if asset_list:
                asset_doc_name = asset_list[0].name
                asset_status = asset_list[0].status
                if asset_status not in ("Scrapped", "Sold", "Cancelled", "Capitalized"):
                    try:
                        from erpnext.assets.doctype.asset.depreciation import scrap_asset
                        scrap_asset(asset_doc_name)
                        asset_result = {"asset": asset_doc_name, "scrapped": True}
                    except Exception as e:
                        frappe.log_error(
                            f"Failed to scrap Asset {asset_doc_name} for Sheep {animal_name}: {e}",
                            "cull_animal"
                        )
                        asset_result = {"asset": asset_doc_name, "scrapped": False, "error": str(e)}
                else:
                    asset_result = {"asset": asset_doc_name, "scrapped": False, "reason": f"Already {asset_status}"}
            else:
                asset_result = {"asset": None, "scrapped": False, "reason": "No submitted Asset found"}
            ...
            frappe.log_error(
                f"Error culling Sheep {animal_name}: {e}",
                "cull_animal"
            )
            results.append({
                "animal": animal_name,
...
def create_stock_ledger_entry_collections(animal_product, qty_issued, reference_doctype=None, reference_name=None):
    ...
    sle_doc = frappe.get_doc({
        ...
        "voucher_type": reference_doctype or "Sheep",
...
def get_collection_data(sheep):
    ...
    try:
        ...
        doc = frappe.get_doc("Sheep", sheep)
        ...
        frappe.log_error(f"Attempted to get collection data for non-existent batch: {sheep}")
...
@frappe.whitelist()
def get_treatment_chart_data(sheep: str):
    """
    ...
    logs = frappe.get_all(
        ...
        filters={"specific_cattle_under_treatment": sheep},
        fields=[
            "name",
            "treatment_date",
            "vaccine_used",

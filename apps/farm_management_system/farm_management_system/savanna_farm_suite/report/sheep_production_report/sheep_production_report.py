...
def execute(filters=None):
    ...
    columns = [
        "Sheep:Link/Sheep:180",
        "Sheep Nickname:Data:140",
    ...
    sheep_filter = filters.get("sheep")
    product_filter = filters.get("product")

    # aggregation: key = (sheep_name, iso_date, product_collected, uom) -> total_qty
    agg = {}

    # Preload list of sheep (narrow by cow_filter if provided)
    if sheep_filter:
        sheep_list = frappe.get_all("Sheep", filters={"name": sheep_filter}, fields=["name"])
    else:
        sheep_list = frappe.get_all("Sheep", fields=["name"])

    # We'll cache nicknames per sheep to avoid repeated fetches later
    nickname_map = {}

    for c in sheep_list:
        try:
            sheep_doc = frappe.get_doc("Sheep", c.name)
        except Exception:
            continue

        # cache nickname
        nickname_map[c.name] = (sheep_doc.get("add_nickname_optional") or "") if hasattr(sheep_doc, "get") else (getattr(sheep_doc, "add_nickname_optional", "") or "")

        rows = getattr(sheep_doc, "production_log", []) or []
        for row in rows:
    ...
    for key in sorted_keys:
        sheep_name, iso_date, product_collected, uom = key
        total_qty = agg[key]
        ...
        # Sheep nickname from cache (may be empty string)
        nickname = nickname_map.get(sheep_name, "")

        # Append row in the same order as columns
        data.append([
            sheep_name,          # Sheep (Link)
            nickname,          # Sheep Nickname (Data)
